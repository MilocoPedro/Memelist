...
export function useShoppingData(user: { uid: string; email: string; displayName?: string } | null, isMockUser = false) {
  ...
  if (detail?.code !== 'permission-denied') {
    console.warn("Activating local fallback (network error):", detail);
    setUseLocalOverride(true);
  } else {
    console.error("🔴 Firestore permission-denied — verifica las reglas en Firebase Console. NO activando fallback local.");
  }
};
  ...
  const addShoppingItem = async (
    ...
    if (isFirebaseConfigured && db && !isLocalMode) {
      ...
      try {
        await setDoc(doc(db, 'lists', activeListId, 'items', itemId), newItem);
      } catch (error) {
        handleFirestoreError(error, OperationType.CREATE, path);
      }
    }
