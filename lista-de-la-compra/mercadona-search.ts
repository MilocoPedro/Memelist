...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ── Intento 1: API live de Mercadona (flujo de dos pasos) ──────────────────
  try {
    ...
    if (warehouseRes.ok) {
      ...
      // La API puede devolver un objeto directo o un array
      const warehouseId: string | null =
        warehouseData?.id ?? warehouseData?.[0]?.id ?? null;

      if (warehouseId) {
        // Paso 2 — búsqueda con el warehouse correcto
          ...
          const mapped = rawProducts
            .map((item: any) => {
              ...
              if (item.price_instructions) {
                price = parseFloat(
                  item.price_instructions.unit_price ?? item.price_instructions.price
                );
  ...
  // ── Fallback local con búsqueda fuzzy por score ────────────────────────────
